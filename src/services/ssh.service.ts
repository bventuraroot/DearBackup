...
export interface SSHClientConfig {
  ...
  port: number;
  username: string;
  authType: 'key' | 'custom_key' | 'password';
  password?: string;
  privateKey?: string;
  passphrase?: string;
}
...
export class SSHService {
   ...
   */
  public static buildConnectConfig(config: SSHClientConfig): ConnectConfig {
    ...
    if (config.authType === 'password' && config.password) {
      connectConfig.password = VaultService.decrypt(config.password);
      connectConfig.tryKeyboard = true;
    } else if (config.authType === 'custom_key' || config.privateKey) {
      if (config.privateKey) {
        connectConfig.privateKey = VaultService.decrypt(config.privateKey);
      }
      if (config.passphrase) {
        connectConfig.passphrase = VaultService.decrypt(config.passphrase);
      } else if (config.password) {
        connectConfig.passphrase = VaultService.decrypt(config.password);
      }
    } else {
      // Usar la llave SSH por defecto del sistema
      const systemKey = VaultService.getOrCreateSystemSSHKey();
   ...
   */
  public static async testSSHConnection(config: SSHClientConfig): Promise<{ success: boolean; message: string; osInfo?: string }> {
   ...
   */
  public static async testDatabaseConnection(
    sshConfig: SSHClientConfig,
    dbType: 'mysql' | 'postgres' | 'none',
    connectionMode: 'ssh_tunnel' | 'docker_container' | 'direct_tcp' = 'ssh_tunnel',
    ...
    const dbPass = VaultService.decrypt(dbPassEncrypted);
  ...
  public static async executeCommand(
    ...
    command: string,
    onLog?: (type: 'stdout' | 'stderr' | 'info', data: string) => void
